...
def test_dataset(model, dataset,uuid): #nr_images):
    ...
    image = np.array(Image.open('./test_images/img_'+uuid+'.jpg'))
    ...
    print(r_fused['class_ids'])
    print([dataset.class_names[id] for id in r_fused['class_ids']])
    print(r_fused)
    ...
    r_fused['class_names'] = dataset.class_names
    encodedNumpyData = json.dumps(r_fused, cls=NumpyArrayEncoder)

    with open('./test_images/output_'+uuid+'.json', 'w') as fp:
        json.dump(encodedNumpyData, fp, sort_keys=True, indent=4)
    #visualize.display_instances(image, r['rois'], r['masks'], r['class_ids'],
    #                            dataset.class_names, r['scores'], title="Predictions", ax=ax1,show_mask=False)

    #visualize.display_instances(image, r_fused['rois'], r_fused['masks'], r_fused['class_ids'],
    #                                dataset.class_names, r_fused['scores'], title="Predictions fused", ax=ax2,show_mask=False)
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--image', required=True, metavar="/path/file.jpg", help=' Target classes')
    ...
    if args.command == "train":
        ...
        test_dataset(model, dataset_test,uuid)#len(dataset_test.image_ids))
    else:
